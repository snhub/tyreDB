...
class Tyres extends Migration
{
	...
	public function up()
	{
		Schema::create($this->tableName, function (Blueprint $table) {
			...
			$table->integer('model_id')->unsigned()->comment('tyre_models->id');
			$table->foreign('model_id')->references('id')->on('tyre_models');
			$table->integer('axle_id')->unsigned()->comment('tyre_axles->id');
			$table->foreign('axle_id')->references('id')->on('tyre_axles');
			$table->integer('width_id')->unsigned()->comment('tyre_widths->id');
			...
			$table->decimal('diameter', 3, 1)->comment('Durchmesser');
			$table->integer('tyre_load_id')->unsigned()->comment('tyre_load_indices->id');
			$table->foreign('tyre_load_id')->references('id')->on('tyre_load_indices');
			$table->integer('tyre_speed_class_id')->unsigned()->comment('tyre_speed_classes->id');
			...
			$table->integer('location_id')->unsigned()->comment('tyre_locations->id');
